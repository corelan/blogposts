...
def main():
    ...
    try:
        with open(bp_path, "w", encoding="utf-8") as bf:
            bf.write(f'.printf "corelan_trigscan: setting {len(bp_cmds)} breakpoints\\n"\n')
            for bp_cmd in bp_cmds:
                bf.write(bp_cmd + "\n")
            bf.write(f'.printf "corelan_trigscan: created {len(bp_cmds)} breakpoints\\n"\n')
    except Exception as e:
    ...
    try:
        for chunk_index, start in enumerate(range(0, len(bp_cmds), chunk_size), start=1):
            ...
            with open(chunk_path, "w", encoding="utf-8") as cf:
                cf.write(f'.printf "corelan_trigscan: setting {len(chunk)} breakpoints from chunk {chunk_index}\\n"\n')
                for bp_cmd in chunk:
                    cf.write(bp_cmd + "\n")
                cf.write(f'.printf "corelan_trigscan: created {len(chunk)} breakpoints from chunk {chunk_index}\\n"\n')
